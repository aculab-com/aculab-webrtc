...
export class TokenRegisterer {
  ...
  constructor(userAgent: UserAgent, options: RegistererOptions = {}) {
    ...
    this.options = Object.assign(
      Object.assign(Object.assign({}, TokenRegisterer.defaultOptions()), {
        registrar: defaultUserAgentRegistrar,
      }),
      TokenRegisterer.stripUndefinedProperties(options),
    );
    ...
    const fromURI =
      ...
      userAgent.userAgentCore.configuration.aor;
    const toURI =
    ...
    const outgoingRegisterRequest = this.userAgent.userAgentCore.register(
      this.request,
      {
        ...
        onReject: response => {
          ...
          this.logger.warn(
            `Failed to register, status code ${response.message.statusCode}`,
          );
          ...
          this._retryAfter = isNaN(retryAfterDuration)
            ? undefined
            : retryAfterDuration;
  ...
  waitingToggle(waiting: boolean) {
    if (this._waiting === waiting) {
      throw new Error(
        `Invalid waiting transition from ${this._waiting} to ${waiting}`,
      );
    }
