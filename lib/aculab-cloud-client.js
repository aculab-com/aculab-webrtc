...
class AculabCloudClient {
        ...
        this._ua.delegate = {
            ...
            onInvite: (invitation) => {
                ...
                if (this._calls.size >= this.maxConcurrent) {
                    ...
                    if (this.onIncoming) {
                        const ic = new aculab_cloud_incoming_call_1.AculabCloudIncomingCall(this, invitation); // TODO: see how looks like the invitation parameter
                        ...
                        try {
                            ...
                            this.console_error('AculabCloudClient onIncoming cause exception: ' + err.message);
    ...
    sessionDescriptionHandlerFactory(session, options) {
        ...
        let sessionDescriptionHandlerConfiguration = {
            iceGatheringTimeout,
            peerConnectionConfiguration: Object.assign(Object.assign({}, sip_js_1.Web.defaultPeerConnectionConfiguration()), options === null || options === void 0 ? void 0 : options.peerConnectionConfiguration)
        };
        ...
        this._option_request = core.request(message, {
            onAccept: (response) => {
                ...
                try {
                    ...
                    this.console_error('AculabCloudClient: failed to parse iceServers response: ' + err.message);
                    this._aculabIceServers = undefined;
                }
    ...
    callService(serviceName) {
        ...
        var outcall = new aculab_cloud_outgoing_service_call_1.AculabCloudOutgoingServiceCall(this, serviceName);
    ...
    callClient(clientId, token, options) {
        ...
        var outcall = new aculab_cloud_outgoing_client_call_1.AculabCloudOutgoingClientCall(this, clientId, token, options);
        this._calls.add(outcall);
        return outcall;
    }
    enableIncoming(token) {
        ...
        if (!this._registerer) {
            ...
            this._registerer.stateChange.addListener((update) => {
                ...
                var ready = (update.state == sip_js_1.RegistererState.Registered);
                if (this.onIncomingState) {
                    ...
                    try {
                        ...
                        this.console_error(`AculabCloudCaller onIncomingState(${ready}, ${update.cause}, ${retry}) caused exception: ${err.message}`);
                    }
                }
