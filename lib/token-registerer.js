...
const sip_js_2 = require("sip.js");
const sip_js_3 = require("sip.js");
const sip_js_4 = require("sip.js");
...
class TokenRegisterer {
    ...
    constructor(userAgent, options = {}) {
        ...
        this._state = sip_js_4.RegistererState.Initial;
        /** True is waiting for final response to outstanding REGISTER request. */
        this._waiting = false;
        // state emitter
        this._stateEventEmitter = new sip_js_2.EmitterImpl();
        // waiting emitter
        this._waitingEventEmitter = new sip_js_2.EmitterImpl();
        // Set user agent
        ...
        if (this.options.instanceId && sip_js_1.Grammar.parse(this.options.instanceId, "uuid") === -1) {
    ...
    dispose() {
        ...
        return new Promise((resolve) => {
            const doClose = () => {
                ...
                if (!this.waiting && this._state === sip_js_4.RegistererState.Registered) {
     ...
     */
    register(options) {
        if (this.state === sip_js_4.RegistererState.Terminated) {
            this.stateError();
        ...
        if (this.waiting) {
            this.waitingWarning();
            const error = new sip_js_3.RequestPendingError("REGISTER request already in progress, waiting for final response");
    ...
    unregister(options) {
        ...
        if (this.disposed) {
            if (this.state !== sip_js_4.RegistererState.Registered) {
        ...
        if (this._state !== sip_js_4.RegistererState.Registered && !options?.all) {
    ...
    registered(expires) {
        ...
        this.stateTransition(sip_js_4.RegistererState.Registered, "NORMAL");
    }
    /**
    ...
    unregistered(reason) {
        this.clearTimers();
        this.stateTransition(sip_js_4.RegistererState.Unregistered, reason);
    }
    /**
    ...
    terminated() {
        this.clearTimers();
        if (this._state !== sip_js_4.RegistererState.Terminated) {
            this.stateTransition(sip_js_4.RegistererState.Terminated);
        }
    }
    /**
    ...
    stateTransition(newState, reason) {
        ...
        var notify = true;
        switch (this._state) {
            case sip_js_4.RegistererState.Initial:
                if (newState !== sip_js_4.RegistererState.Registered &&
                    newState !== sip_js_4.RegistererState.Unregistered &&
                    newState !== sip_js_4.RegistererState.Terminated) {
                    invalidTransition();
                }
                break;
            case sip_js_4.RegistererState.Registered:
                if (newState === sip_js_4.RegistererState.Registered) {
                    notify = this.force_notify;
                }
                else if (newState !== sip_js_4.RegistererState.Unregistered && newState !== sip_js_4.RegistererState.Terminated) {
                    invalidTransition();
                }
                break;
            case sip_js_4.RegistererState.Unregistered:
                if (newState === sip_js_4.RegistererState.Unregistered) {
                    notify = this.force_notify;
                }
                else if (newState !== sip_js_4.RegistererState.Registered && newState !== sip_js_4.RegistererState.Terminated) {
                    invalidTransition();
                }
                break;
            case sip_js_4.RegistererState.Terminated:
                invalidTransition();
                break;
            default:
        ...
        // Dispose
        if (newState === sip_js_4.RegistererState.Terminated) {
            this.dispose();
        }
    }
    ...
    /** Hopefully helpful as the standard behavior has been found to be unexpected. */
    stateError() {
        const reason = this.state === sip_js_4.RegistererState.Terminated ? "is in 'Terminated' state" : "has been disposed";
