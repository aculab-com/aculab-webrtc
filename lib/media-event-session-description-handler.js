...
class MediaEventSessionDescriptionHandler extends sip_js_1.Web.SessionDescriptionHandler {
    options;
    usingOptionsLocalStream;
    acuLocalMediaStreams;
    _acuRemoteMediaStreams;
    _acuGUMStream;
    notified_streams;
    userToInternalLocalStreamIds;
    remoteMediaStreamsToInternal;
    onUserMedia;
    onUserMediaRemove;
    onUserMediaFailed;
    ...
    setRemoteTrack(track) {
      ...
      */
            ...
            sip_js_1.Web.SessionDescriptionHandler.dispatchAddTrackEvent(remoteStream, track); // TODO investigate this while testing !!
        }
    ...
    setLocalMediaStream(stream) {
        ...
        let exists = false;
        this.logger.debug("SessionDescriptionHandler.setLocalMediaStream: Finding stream " +
            stream.id);
        this.acuLocalMediaStreams.forEach(stm => {
            this.logger.debug("SessionDescriptionHandler.setLocalMediaStream: Checking stream " +
                stream.id);
            if (stream.id == stm.id) {
                this.logger.debug("SessionDescriptionHandler.setLocalMediaStream: Stream already exists " +
                    stream.id);
                exists = true;
        ...
        if (!exists) {
            ...
            this.logger.debug("SessionDescriptionHandler.setLocalMediaStream: Adding audio tracks " +
                stream.id);
            ...
            this.logger.debug("SessionDescriptionHandler.setLocalMediaStream: Adding video tracks " +
                stream.id);
    ...
    removeLocalMediaStream(stream) {
        ...
        this.acuLocalMediaStreams = this.acuLocalMediaStreams.filter(s => s.id != stream.id);
    }
    addStreamToInternalList(stream, do_clone) {
        ...
        try {
            ...
            if (options.localStreams !== undefined) {
                ...
                if (this._acuGUMStream == null) {
                    this._acuGUMStream = await navigator.mediaDevices.getUserMedia(options.constraints);
                }
        ...
        const updateTransceiverCodecsAndBitrates = (transceiver, kind) => {
            ...
            if (transceiver.sender) {
                ...
                if (bitrate) {
                    ...
                    if (changed) {
                        this.logger.debug("SessionDescriptionHandler.updateDirection - setting " +
                            kind +
                            " bandwidth");
                        transceiver.sender
